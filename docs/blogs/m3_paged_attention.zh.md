...
> **数据来源**(均在 [`benchmarks/results/m3/`](../../benchmarks/results/m3/),同一 session):
> M1 `nano_vllm_m1_20260711_012918.json`、M2 `nano_vllm_m2_20260711_013719.json`、
> M3 `nano_vllm_m3_20260711_014557.json`,字段 `avg_tpot` / `avg_ttft` / `output_throughput`
> (M3 TTFT 秒,短→长:0.0269 / 0.0306 / 0.0917)。长 prompt 的 TTFT 明显更高,是因为 prefill 要一次算完整个长 prompt。
...
> **数据来源**:平均长度取自 M3 [`nano_vllm_m3_20260711_014557.json`](../../benchmarks/results/m3/nano_vllm_m3_20260711_014557.json)
> (`avg_prompt_len + avg_output_len`);MB 由 `benchmarks/kv_footprint.py --block-size 256` 按公式算出
> (M2 = 固定 `max_seq_len` 8192)。
